...
G4VPhysicalVolume *LHDetectorConstruction::Construct()
{
	...
	if ( par -> GetParBool("FTIn") )
	{
		...
		G4int    ftFiberN = par->GetParInt("FTFiberN");    //Total # of fibers
		...
		G4double ftBoxW  = par->GetParDouble("FTBoxW"); //layer Width (x)
		G4double ftBoxH  = par->GetParDouble("FTBoxH"); //layer Height (y)
		G4double ftBoxT  = par->GetParDouble("FTBoxT"); //layer Thickness (z)
		G4double ftFiberW  = par->GetParDouble("FTFiberW"); //fiber Width (x)
		G4double ftFiberH  = par->GetParDouble("FTFiberH"); //fiber Height (y)
		G4double ftFiberT  = par->GetParDouble("FTFiberT"); //fiber Thickness (z)
		...
		G4RotationMatrix* RotXY90 = new G4RotationMatrix();
        RotXY90->rotateX(90 * deg).rotateY(90 * deg);
		...
		// G4Box* FTSolidBox = new G4Box("FTSolidBox", ftBoxW/2., ftBoxH/2., limitT/2.);
		// G4Box* FTSolidHole = new G4Box("FTSolidHole", ftHoleW/2., ftHoleH/2., limitT/2.);

		// create the scintillator fiber shape
		G4Box* FTfiberShape = new G4Box("FTfiberShape", limitT, ftFiberW/2., ftFiberH/2.);
		G4LogicalVolume* FTLogic = new G4LogicalVolume(FTfiberShape, matSC, "FTLogic");
		FTLogic->SetVisAttributes(G4VisAttributes::GetInvisible());




		// Create a visualization attribute for the scintillator fiber
		G4VisAttributes* FTvisual = new G4VisAttributes(G4Colour(G4Colour::Cyan()));
		FTvisual->SetForceWireframe(true);
		//G4VisAttributes* FTvisual2 = new G4VisAttributes(G4Colour(G4Colour::Magenta()));
		//FTvisual2->SetForceWireframe(true);

		//Implementation, each unit
		for (int a=0; a<ftLayerN; a++)
		{
			// G4ThreeVector ftUnitOfs(0, 0, limitT/(ftLayerN-1)*a - limitT/2.);
			// auto FT_unit = new G4PVPlacement(0, ftUnitOfs, FTLogicUnit, Form("FTracker_L%d", a), FTLogic, false, 100*ftID+a, false);
			for (int i=0; i<ftFiberN; i++) //! Scintillator Fiber constructoin
			{
				// G4ThreeVector ftFibUnitXOfs(-ftBoxW/2., ftBoxH*(1/2-1/(2*ftFiberN)-i/ftFiberN), limitT/(ftLayerN-1)*a - limitT/2.);
				// G4ThreeVector ftFibUnitXOfs(0, 0, limitT/(ftLayerN-1)*a - limitT/2.);
					// Create a placement for the scintillator fiber
					//G4ThreeVector ftFibUnitXOfs(0, 0, 0);
					G4ThreeVector ftFibUnitXOfs(0, ftBoxH/2 - ftBoxH*i/(ftFiberN-1), limitT/(ftLayerN-1)*a - 3*limitT/2. - 100);
					//G4ThreeVector ftFibUnitXOfs(0, 2*i , limitT/(ftLayerN-1)*a - 3*limitT/2.);
					G4ThreeVector ftFibUnitYOfs(ftBoxH/2 - ftBoxH*i/(ftFiberN-1), 0, limitT/(ftLayerN-1)*a - 3*limitT/2. - 105);
					//G4ThreeVector ftFibUnitXOfs(0, 0, 0);
					// auto FTfiberPlacementX+Form("%d_%d",{a,i}) = new G4PVPlacement(RotX90, ftFibUnitXOfs, FTLogic, "FTfiberPlacement", logicWorld, false, 0, true);
					// auto FTfiberPlacementY+Form("%d_%d",{a,i}) = new G4PVPlacement(RotXY90, ftFibUnitYOfs, FTLogic, "FTfiberPlacement2", logicWorld, false, 0, true);

				//auto FTfiberPlacementX = new G4PVPlacement(RotX90, ftFibUnitXOfs, FTLogic, Form("FTfiberPlacement_L%dF%d_X", a, i), logicWorld, false, 10000*ftID+100*a+i, true);
				auto FTfiberPlacementX = new G4PVPlacement(0, ftFibUnitXOfs, FTLogic, Form("FTfiberPlacement_L%dF%d_X", a, i), logicWorld, false, 10000*ftID+100*a+i, true);
				auto FTfiberPlacementY = new G4PVPlacement(RotZ90, ftFibUnitYOfs, FTLogic, Form("FTfiberPlacement_L%dF%d_Y", a, i), logicWorld, false, 10000*ftID+100*a+i, true);
				FTfiberPlacementX->GetLogicalVolume()->SetVisAttributes(FTvisual);
				//FTfiberPlacementY->GetLogicalVolume()->SetVisAttributes(FTvisual2);
			}
		}//a
		...
		// G4ThreeVector ftFibUnitXOfs(0./*-ftBoxW/2*/, ftBoxH*(1/2-1/(2*ftFiberN)-1/*i*//ftFiberN), limitT/(ftLayerN-1)*1/*a*/ - limitT/2.);
		// auto FT_fib_unitX = new G4PVPlacement(0, ftFibUnitXOfs, FTFiberLogicUnit, Form("FTracker_L_F"), logicWorld, false, 10000*ftID+1, true);
		// auto FT_fib_unitX = new G4PVPlacement(0, ftFibUnitXOfs, FTLogicUnit, Form("FTracker_L%dF%d_X", a, i), logicWorld, false, 10000*ftID+100*a+i, true);
		// auto FT_fib_unitY = new G4PVPlacement(RotZ90, ftFibUnitXOfs, FTLogicUnit, Form("FTracker_L%dF%d_Y", a, i), FTLogic, false, 10000*ftID+100*a+i, true);

		//Implementation, mother (whole)
		//G4ThreeVector FTOfs(0, 0, ftLayerZ + totalT/2.);
		// G4ThreeVector FTOfs(0, 0, ftLayerZ + limitT/2.);
		// auto FT = new G4PVPlacement(0, FTOfs, FTLogic, "FTracker", logicWorld, false, ftID, true);
		// runManager->SetSensitiveDetector(FT_fib_unitX);
	}//FT, rectangle
	#endif
